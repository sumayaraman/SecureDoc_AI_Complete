'use client';
import {useState} from 'react';import {useRouter} from 'next/navigation';import Link from 'next/link';import {ShieldCheck} from 'lucide-react';import {api} from '../../lib/api';
export default function Login(){
  const [email,setEmail]=useState('demo@securedoc.local'),[password,setPassword]=useState('Demo@123456'),[error,setError]=useState(''),[busy,setBusy]=useState(false);
  const r=useRouter();
    ...
    <main className="min-h-screen bg-slate-50 grid place-items-center p-6">
      <form onSubmit={submit} className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <b>SecureDoc AI</b>
            <p className="text-xs text-slate-400">Private document intelligence</p>
          </div>
        </div>
        ...
        <label className="mt-6 block text-sm font-medium">
          Email
          <input className="mt-2 w-full rounded-xl border p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500" value={email} onChange={e=>setEmail(e.target.value)}/>
        </label>
        <label className="mt-4 block text-sm font-medium">
          Password
          <input type="password" className="mt-2 w-full rounded-xl border p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500" value={password} onChange={e=>setPassword(e.target.value)}/>
        </label>
        ...
        <p className="mt-5 text-center text-sm text-slate-500">
          New here? <Link className="font-semibold text-indigo-600" href="/register">Create an account</Link>
        </p>
        <div className="mt-5 rounded-xl bg-slate-50 p-3 text-xs text-slate-500">
          Demo login: demo@securedoc.local / Demo@123456
        </div>
      </form>
    </main>
  );
